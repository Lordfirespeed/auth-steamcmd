...
export class AuthenticateSteamCMD {
  ...
  async run(): Promise<void> {
    const task = pipe(
      ...
      TE.bind("steamConfigDirectory", state=> this.ensureSteamConfigDirTaskEither(state)),
      TE.bind("steamConfigFile", state => this.writeSteamConfigFileTaskEither(state)),
  ...
  async ensureSteamConfigDir(steamHome: string) {
    ...
    try {
      ...
      core.error(`Couldn't ensure steam config directory. Reason: ${discernFileSystemErrorReason(error, { file: steamConfigDir })}`)
      throw error
    }
  }

  writeSteamConfigFileTaskEither({steamConfigDirectory, configValveDataFormat}: {steamConfigDirectory: string, configValveDataFormat: Buffer}) {
    return TE.tryCatch(
  ...
  async writeSteamConfigFile(steamConfigDirectory: string, configValveDataFormat: Buffer) {
    ...
    try {
      ...
      await steamConfigFileHandle.writeFile(configValveDataFormat, { encoding: "ascii" })
      core.info("Done.")
      return steamConfigFile
    }
    catch (error) {
      core.error(`Failed to write Steam config. Reason: ${discernFileSystemErrorReason(error, { file: steamConfigFile })}`)
      throw error
    }
  }
  ...
  async testLoginSucceeds(steamUsername: string) {
    ...
    const loginExitCode = await exec.exec(
      "steamcmd",
      ["+login", "+quit"],
      { ignoreReturnCode: true, input: Buffer.from("\u0004") }
    )
