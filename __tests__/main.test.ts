...
let getInputsMock: jest.SpyInstance<ReturnType<AuthenticateSteamCMD["getInputs"]>, Parameters<AuthenticateSteamCMD["getInputs"]>>
let writeFileMock: jest.SpyInstance<ReturnType<AuthenticateSteamCMD["writeFile"]>, Parameters<AuthenticateSteamCMD["writeFile"]>>
let expandEnvVarsMock: jest.SpyInstance<ReturnType<AuthenticateSteamCMD["expandEnvVars"]>, Parameters<AuthenticateSteamCMD["expandEnvVars"]>>
...
describe('action', () => {
  ...
  beforeEach(() => {
    ...
    fsMkdirMock = jest.spyOn(fs, "mkdir").mockImplementation(
      (filename: PathLike, options: Parameters<typeof fs.mkdir>[1]) => Promise.resolve(filename.toString())
    )
    ...
    fsOpenMock = jest.spyOn(fs, "open").mockImplementation(
  ...
  describe("getInputs", () => {
    ...
    it("rejects when provided steam_config_vdf input that is not base64 encoded", async () => {
      mockInputs(new Map()
        .set("steam_config_vdf", "Really really not base64 encoded, lol")
      )
